...
DefinePyFunction(ocVBMManager, makeAllChildRelations) {
    ...
    ObjRef(self, ocVBMManager)->makeAllChildRelations(rel->obj, rels, bMakeProject);
...
DefinePyFunction(ocVBMManager, makeChildModel) {
    ...
    ocModel *ret = ObjRef(self, ocVBMManager)->makeChildModel(Pmodel->obj, relation, &cache, bMakeProject);
    if (ret == NULL)
...
DefinePyFunction(ocVBMManager, printOptions) {
    ...
    ObjRef(self, ocVBMManager)->printOptions(printHTML != 0, skipNominal != 0);
    Py_INCREF(Py_None);
    return Py_None;
}
...
static struct PyMethodDef ocVBMManager_methods[] = { PyMethodDef(ocVBMManager, initFromCommandLine),
        ...
        PyMethodDef(ocVBMManager, searchOneLevel), PyMethodDef(ocVBMManager, setSearchType),
        PyMethodDef(ocVBMManager, getTopRefModel), PyMethodDef(ocVBMManager, getBottomRefModel),
        PyMethodDef(ocVBMManager, getRefModel), PyMethodDef(ocVBMManager, setRefModel),
        PyMethodDef(ocVBMManager, computeDF), PyMethodDef(ocVBMManager, computeH), PyMethodDef(ocVBMManager, computeT),
        PyMethodDef(ocVBMManager, computeInformationStatistics), PyMethodDef(ocVBMManager, computeDFStatistics),
        PyMethodDef(ocVBMManager, computeL2Statistics), PyMethodDef(ocVBMManager, computePearsonStatistics),
        ...
        PyMethodDef(ocVBMManager, makeFitTable), PyMethodDef(ocVBMManager, isDirected),
        PyMethodDef(ocVBMManager, printOptions), PyMethodDef(ocVBMManager, deleteTablesFromCache),
...
/****** Type Definition ******/
PyTypeObject TocVBMManager = { PyObject_HEAD_INIT(&PyType_Type) 0, "ocVBMManager",
sizeof(PocVBMManager), 0,
//-- standard methods
...
// ocModel **searchOneLevel(ocModel *)
DefinePyFunction(ocSBMManager, searchOneLevel) {
    PocModel *start;
    PocModel *pmodel;
    PyArg_ParseTuple(args, "O!", &TocModel, &start);
    ocModel **models;
...
// void setSearchType(const char *name)
DefinePyFunction(ocSBMManager, setSearchType) {
    char *name;
    PyArg_ParseTuple(args, "s", &name);
    ocSBMManager *mgr = ObjRef(self, ocSBMManager);
    mgr->setSearch(name);
...
// long computeDF(ocModel *model)
DefinePyFunction(ocSBMManager, computeDF) {
    PyObject *Pmodel;
    PyArg_ParseTuple(args, "O!", &TocModel, &Pmodel);
    ocModel *model = ObjRef(Pmodel, ocModel);
    double df = ObjRef(self, ocSBMManager)->computeDfSb(model);
    return Py_BuildValue("d", df);
}
...
// void computeIncrementalAlpha(ocModel *model)
DefinePyFunction(ocSBMManager, computeIncrementalAlpha) {
    PyObject *Pmodel;
    PyArg_ParseTuple(args, "O!", &TocModel, &Pmodel);
    ocModel *model = ObjRef(Pmodel, ocModel);
    if (model == NULL)
        onError("Model is NULL!");
    ObjRef(self, ocSBMManager)->computeIncrementalAlpha(model);
    Py_INCREF(Py_None);
    return Py_None;
}

// void compareProgenitors(ocModel *model, ocModel *newProgen)
DefinePyFunction(ocSBMManager, compareProgenitors) {
    PyObject *Pmodel, *Pprogen;
    PyArg_ParseTuple(args, "O!O!", &TocModel, &Pmodel, &TocModel, &Pprogen);
    ocModel *model = ObjRef(Pmodel, ocModel);
    ocModel *progen = ObjRef(Pprogen, ocModel);
    ...
    if (progen == NULL)
        onError("Progen is NULL.");
    ObjRef(self, ocSBMManager)->compareProgenitors(model, progen);
    Py_INCREF(Py_None);
    return Py_None;
}
...
// void setSearchDirection(int dir)
// 0 = up, 1 = down
DefinePyFunction(ocSBMManager, setSearchDirection) {
    int dir;
    PyArg_ParseTuple(args, "i", &dir);
    ObjRef(self, ocSBMManager)->setSearchDirection(dir);
    Py_INCREF(Py_None);
    return Py_None;
}
...
DefinePyFunction(ocSBMManager, getOptionList) {
    ...
    void *nextp = NULL;
    PyObject *list = PyList_New(0);
    while (ObjRef(self, ocSBMManager)->getOptionString(attrName, &nextp, &value) && nextp != NULL) {
        PyObject *valstr = Py_BuildValue("s", value);
        PyList_Append(list, valstr);
    }
    Py_INCREF(list);
...
DefinePyFunction(ocSBMManager, printOptions) {
    ...
    int skipNominal;
    PyArg_ParseTuple(args, "ii", &printHTML, &skipNominal);
    ObjRef(self, ocSBMManager)->printOptions(printHTML != 0, skipNominal != 0);
    Py_INCREF(Py_None);
    return Py_None;
}

// bool deleteModelFromCache(ocModel *model)
DefinePyFunction(ocSBMManager, deleteModelFromCache) {
    PyObject *Pmodel;
    PyArg_ParseTuple(args, "O!", &TocModel, &Pmodel);
    ocModel *model = ObjRef(Pmodel, ocModel);
    if (model == NULL)
        onError("Model is NULL!");
    bool success = ObjRef(self, ocSBMManager)->deleteModelFromCache(model);
    return Py_BuildValue("i", success ? 1 : 0);
}
...
static struct PyMethodDef ocSBMManager_methods[] = { PyMethodDef(ocSBMManager, initFromCommandLine),
        ...
        PyMethodDef(ocSBMManager, getTopRefModel), PyMethodDef(ocSBMManager, getBottomRefModel),
        PyMethodDef(ocSBMManager, getRefModel), PyMethodDef(ocSBMManager, setRefModel),
        PyMethodDef(ocSBMManager, computeDF), PyMethodDef(ocSBMManager, computeH), PyMethodDef(ocSBMManager, computeT),
        PyMethodDef(ocSBMManager, computeInformationStatistics), PyMethodDef(ocSBMManager, computeDFStatistics),
        PyMethodDef(ocSBMManager, computeL2Statistics), PyMethodDef(ocSBMManager, computePearsonStatistics),
        PyMethodDef(ocSBMManager, computeDependentStatistics), PyMethodDef(ocSBMManager, computeBPStatistics),
        PyMethodDef(ocSBMManager, computeIncrementalAlpha), PyMethodDef(ocSBMManager, compareProgenitors),
        PyMethodDef(ocSBMManager, setSearchDirection), PyMethodDef(ocSBMManager, printFitReport),
        PyMethodDef(ocSBMManager, getOption), PyMethodDef(ocSBMManager, getOptionList),
...
/****** Type Definition ******/
PyTypeObject TocSBMManager = { PyObject_HEAD_INIT(&PyType_Type) 0, "ocSBMManager",
sizeof(PocSBMManager), 0,
//-- standard methods
...
static struct PyMethodDef ocRelation_methods[] = { PyMethodDef(ocRelation, get), { NULL, NULL, 0 } };
...
/****** Type Definition ******/
PyTypeObject TocRelation = { PyObject_HEAD_INIT(&PyType_Type) 0, "ocRelation",
sizeof(PocRelation),
0,
//-- standard methods
        (destructor) 0,
...
// bool isEquivalentTo(ocModel*)
DefinePyFunction(ocModel, isEquivalentTo) {
    PyObject *Pmodel;
    PyArg_ParseTuple(args, "O!", &TocModel, &Pmodel);
    ocModel *other = ObjRef(Pmodel, ocModel);
    ocModel *model = ObjRef(self, ocModel);
...
static struct PyMethodDef ocModel_methods[] = { PyMethodDef(ocModel, getRelation), PyMethodDef(ocModel, get),
        PyMethodDef(ocModel, deleteFitTable), PyMethodDef(ocModel, deleteRelationLinks),
        PyMethodDef(ocModel, setProgenitor), PyMethodDef(ocModel, getProgenitor), PyMethodDef(ocModel, setID),
        PyMethodDef(ocModel, isEquivalentTo), PyMethodDef(ocModel, dump), { NULL, NULL, 0 } };
...
PyTypeObject TocModel = { PyObject_HEAD_INIT(&PyType_Type) 0, "ocModel", sizeof(PocModel), 0,
//-- standard methods
        (destructor) 0,
        (printfunc) 0,
...
/****** Type Definition ******/
PyTypeObject TocReport = { PyObject_HEAD_INIT(&PyType_Type) 0, "ocReport", sizeof(PocReport), 0,
//-- standard methods
        (destructor) 0,
        (printfunc) 0,
...
static struct PyMethodDef occam_methods[] = { { "ocRelation", ocRelation_new, 1 }, { "ocModel", ocModel_new, 1 }, {
        "ocVBMManager", ocVBMManager_new, 1 }, { "ocSBMManager", ocSBMManager_new, 1 },
