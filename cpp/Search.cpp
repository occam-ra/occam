...
// Add this after the existing includes
    ...
    #include <windows.h>
    #include <psapi.h>
    // Windows doesn't have sbrk, so we'll define a simple replacement
    static char* fake_sbrk(int increment) {
        static char* fake_heap = nullptr;
        if (fake_heap == nullptr) {
            fake_heap = (char*)VirtualAlloc(NULL, 1024*1024, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
        }
        return fake_heap;
    }
    #define sbrk(x) fake_sbrk(x)
#else
    #include <unistd.h>  // For sbrk on Unix/Linux
#endif
...
bool SearchChain::makeChainModels() {
    ...
    if (stackPtr >= indVarCount) {
        ...
        // Memory usage tracking - Windows compatible
#ifndef _WIN32
