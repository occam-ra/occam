...
// ========== STORE CONFUSION MATRIX IN MANAGER ==========
    // Persist CM for main model only; do not allow component relations to overwrite
    // This makes CM values outlive the Report object so Python can retrieve them
    if (rel == NULL && model != NULL && manager) {
        auto cm = manager->getMainModelConfusionMatrix();
        
        // First write wins - only store if not already populated (lock pattern)
        if (!cm.has_values) {
            // Store training data CM
            cm.train_tn = trtn;
            cm.train_fp = trfp;
            cm.train_fn = trfn;
            cm.train_tp = trtp;
            ...
            // Store test data CM if available
            if (test_sample_size > 0.0) {
                cm.test_tn = tetn;
                cm.test_fp = tefp;
                cm.test_fn = tefn;
            ...
            fprintf(stderr, "[CM persisted] TN=%g FP=%g FN=%g TP=%g\n", 
                    trtn, trfp, trfn, trtp);
        }
    }
