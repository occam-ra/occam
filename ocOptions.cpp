 ...
 */
ocOptionDef *ocOptions::addOptionName(const char *name, const char *abbrev,
	const char *tip, bool multi)
{
...
bool ocOptions::setOptionString(ocOptionDef *def, const char *value)
{
	...
	//-- the option is not a multiple option, update date it.
...
void ocOptions::write(FILE *fd, bool printHTML)
{
	...
	fprintf(fd, "%sOptions:%s", startline, endline);
	//Anjali 
	while (opt) {
