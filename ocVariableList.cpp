...
int ocVariableList::getNewValue(int index,char * old_value,char*new_value){
    ...
    for (chr = 0; chr < 100; chr++) {
        if (old_value[chr] == '\0' || isspace(old_value[chr]) || old_value[chr] == ',') break;
...
int ocVariableList::getVarStateList(const char *name, int *varlist,int *stlist)
{
    const char *cp = name;
    const char *cp2;
    char vname[MAXNAMELEN];
...
int ocVariableList::getVarValueIndex(int varindex, const char *value)
{
    ...
    for (chr = 0; chr < 100; chr++) {
        if (value[chr] == '\0' || isspace(value[chr]) || (value[chr] == ',')) break;
...
bool ocVariableList::checkCardinalities()
{
    ...
    for (int varindex = 0; varindex < varCount; varindex++) {
        ...
        if (valuecount < cardinality) {
            printf("Warning: input data cardinality for variable %s (%s) is %d, but was specified as %d. The lower value will be used.\n",
            ...
            printf("ERROR: input data cardinality for variable %s (%s) is %d, but was specified as %d. OCCAM will quit.\n",
                    vars[varindex].name, vars[varindex].abbrev, valuecount, cardinality);
            result = false;
        }
    }
