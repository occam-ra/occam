...
static attrDesc attrDescriptions[] = {
    ...
    {ATTRIBUTE_INCR_ALPHA_REACHABLE, "Inc.A.<0.05", "%1.0f"},
...
void ocReport::printConditional_DV(FILE *fd, ocModel *model, ocRelation *rel, bool calcExpectedDV)
{
	...
	test_freq = new int *[iv_statespace];
	test_dv_freq = new int[dv_card];
	test_key_freq = new int[iv_statespace]; 
	...
	fit_dv_prob[i] = 0.0;
	input_dv_freq[i] = 0;
	if(test_sample_size > 0) test_dv_freq[i] = 0;
    }
	...
	fit_tied[i] = false;
	if(test_sample_size > 0) test_rule[i] = input_default_dv;
    }
