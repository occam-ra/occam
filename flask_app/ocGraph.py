...
# PARAMETERS:
    ...
    components = list(filter(lambda s : not (s == 'IVI' or s == 'IV'), components))
    # Split on uppercase letters
    model = list(map(lambda s : re.findall('[A-Z][^A-Z]*', s), components))
    if dvName != "":
        model = list(filter(lambda r : dvName in r, model))
