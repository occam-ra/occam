 ...
 */
long ocReadData(FILE *fin, VariableList *vars, Table *indata, LostVar *lostvarp) {
    char line[MAXLINE];
    KeySegment *key = 0;
    int lineno = 0;
    double tupleValue;
    ...
    int value = 0;
    int l = 0;
    gotLine = Options::getLine(fin, line, &lineno);
    if (!gotLine) {
        printf("No data\n");
        return false;
    }
    while (gotLine) {
        ...
        if (flag == KEEP) {
            Key::buildKey(key, keysize, vars, indices, values, varCount);
...
void ocDefineVariables(Options *options, VariableList *vars) {
...
void ocRebinDefineVar(Options *options, VariableList *vars, LostVar ** lostvarp) {
    //-- Variable def is "name, cardinality, type, abbrev"
 ...
 */
int ocReadFile(FILE *fd, Options *options, Table **indata, Table **testdata, VariableList **vars) {
    VariableList *varp = NULL;
    Table *indatap = NULL;
    ...
    *vars = varp = new VariableList(8);
    ...
    if (!feof(fd)) {
        *indata = indatap = new Table(varp->getKeySize(), 64);
